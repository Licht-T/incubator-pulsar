...
PartitionedConsumerImpl::getConsumerCreatedFuture() {
...
void PartitionedConsumerImpl::unsubscribeAsync(ResultCallback callback) {
  LOG_INFO("[" << destinationName_->toString() << "," << subscriptionName_
               << "] Unsubscribing");
  // change state to Closing, so that no Ready state operation is permitted during
  // unsubscribe
  setState(Closing);
  // do not accept un subscribe until we have subscribe to all of the partitions of a
  // topic
  // it's a logical single topic so it should behave like a single topic, even if it's
  // sharded
  Lock lock(mutex_);
  if (state_ != Ready) {
    lock.unlock();
    ...
    for (ConsumerList::const_iterator consumer = consumers_.begin();
         consumer != consumers_.end(); consumer++) {
      ...
      (*consumer)->unsubscribeAsync(
          boost::bind(&PartitionedConsumerImpl::handleUnsubscribeAsync,
...
void PartitionedConsumerImpl::handleUnsubscribeAsync(Result result,
  ...
  if (state_ == Failed) {
    lock.unlock();
    // we have already informed the client that unsubcribe has failed so, ignore this
    // callbacks
    // or do we still go ahead and check how many could we close successfully?
    LOG_DEBUG(
        "handleUnsubscribeAsync callback received in Failed State for consumerIndex - "
  ...
  // this means we have successfully closed this partition consumer and no unsubscribe has
  // failed so far
  LOG_INFO("Successfully Unsubscribed Consumer - "
           << consumerIndex << " for Subscription - " << subscriptionName_
...
void PartitionedConsumerImpl::acknowledgeCumulativeAsync(const MessageId& msgId,
                                                         ResultCallback callback) {
...
void PartitionedConsumerImpl::start() {
  ...
  for (unsigned int i = 0; i < numPartitions_; i++) {
    ...
    consumer =
        boost::make_shared<ConsumerImpl>(client_, topicPartitionName, subscriptionName_,
                                         config, internalListenerExecutor, Partitioned);
    consumer->getConsumerCreatedFuture().addListener(
        boost::bind(&PartitionedConsumerImpl::handleSinglePartitionConsumerCreated,
                    shared_from_this(), _1, _2, i));
    consumer->setPartitionIndex(i);
    consumers_.push_back(consumer);

    LOG_DEBUG("Creating Consumer for single Partition - "
              << topicPartitionName << "SubName - " << subscriptionName_);
  }
  for (ConsumerList::const_iterator consumer = consumers_.begin();
       consumer != consumers_.end(); consumer++) {
    (*consumer)->start();
...
void PartitionedConsumerImpl::handleSinglePartitionConsumerCreated(
  ...
  if (result != ResultOk) {
    ...
    LOG_DEBUG("Unable to create Consumer for partition - " << partitionIndex
                                                           << " Error - " << result);
    return;
  }
  ...
  if (numConsumersCreated_ == numPartitions_) {
    LOG_INFO("Successfully Subscribed to Partitioned Topic - "
             << destinationName_->toString() << " with - " << numPartitions_
             << " Partitions.");
    state_ = Ready;
    lock.unlock();
    receiveMessages();
...
void PartitionedConsumerImpl::handleSinglePartitionConsumerClose(
    Result result, unsigned int partitionIndex, CloseCallback callback) {
  Lock lock(mutex_);
  if (state_ == Failed) {
...
void PartitionedConsumerImpl::closeAsync(ResultCallback callback) {
  ...
  for (ConsumerList::const_iterator i = consumers_.begin(); i != consumers_.end(); i++) {
    ...
    if (!consumer->isClosed()) {
      consumer->closeAsync(
          boost::bind(&PartitionedConsumerImpl::handleSinglePartitionConsumerClose,
...
void PartitionedConsumerImpl::notifyResult(CloseCallback closeCallback) {
  if (closeCallback) {
    ...
    // consumer create failed, closeAsync called to cleanup the successfully created
    // producers
    setState(Failed);
    partitionedConsumerCreatedPromise_.setFailed(ResultUnknownError);
  }
...
void PartitionedConsumerImpl::getBrokerConsumerStatsAsync(
  ...
  for (int i = 0; i < consumerList.size(); i++) {
    consumerList[i]->getBrokerConsumerStatsAsync(
        boost::bind(&PartitionedConsumerImpl::handleGetConsumerStats, shared_from_this(),
                    _1, _2, latchPtr, statsPtr, i, callback));
  }
}

void PartitionedConsumerImpl::handleGetConsumerStats(
    Result res, BrokerConsumerStats brokerConsumerStats, LatchPtr latchPtr,
