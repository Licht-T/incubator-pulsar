...
HTTPLookupService::HTTPLookupService(const std::string &lookupUrl,
    ...
    : executorProvider_(
          boost::make_shared<ExecutorServiceProvider>(NUMBER_OF_LOOKUP_THREADS)),
...
Future<Result, LookupDataResultPtr> HTTPLookupService::lookupAsync(
  ...
  completeUrlStream << adminUrl_ << V2_PATH << "persistent/" << dn->getProperty() << '/'
                    << dn->getCluster() << '/' << dn->getNamespacePortion() << '/'
...
Future<Result, LookupDataResultPtr> HTTPLookupService::getPartitionMetadataAsync(
  ...
  completeUrlStream << adminUrl_ << PARTITION_PATH << dn->getProperty() << '/'
                    << dn->getCluster() << '/' << dn->getNamespacePortion() << '/'
                    << dn->getEncodedLocalName() << '/' << PARTITION_METHOD_NAME;
  executorProvider_->get()->postWork(
      boost::bind(&HTTPLookupService::sendHTTPRequest, shared_from_this(), promise,
...
void HTTPLookupService::sendHTTPRequest(LookupPromise promise,
  ...
  if (authResult != ResultOk) {
    LOG_ERROR(
        "All Authentication methods should have AuthenticationData and return true on "
        "getAuthData for url "
        << completeUrl);
...
LookupDataResultPtr HTTPLookupService::parseLookupData(const std::string &json) {
  ...
  const std::string brokerUrlSsl =
      root.get("brokerUrlSsl", defaultNotFoundString).asString();
