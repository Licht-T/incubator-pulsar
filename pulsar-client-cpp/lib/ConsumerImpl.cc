...
ConsumerImpl::ConsumerImpl(
    const ClientImplPtr client, const std::string& topic, const std::string& subscription,
    ...
    Commands::SubscriptionMode subscriptionMode, Optional<BatchMessageId> startMessageId)
    : HandlerBase(client, topic,
                  Backoff(milliseconds(100), seconds(60), milliseconds(0))),
  ...
  if (conf.getUnAckedMessagesTimeoutMs() != 0) {
    unAckedMessageTrackerPtr_.reset(new UnAckedMessageTrackerEnabled(
        conf.getUnAckedMessagesTimeoutMs(), client, *this));
  } else {
...
void ConsumerImpl::handleCreateConsumer(const ClientConnectionPtr& cnx, Result result) {
  ...
  if (result == ResultOk) {
    ...
    if (result == ResultTimeout) {
      // Creating the consumer has timed out. We need to ensure the broker closes the
      // consumer
      // in case it was indeed created, otherwise it might prevent new subscribe
      // operation,
    ...
    if (consumerCreatedPromise_.isComplete()) {
      // Consumer had already been initially created, we need to retry connecting in any
      // case
...
void ConsumerImpl::unsubscribeAsync(ResultCallback callback) {
  ...
  if (state_ != Ready) {
    ...
    LOG_ERROR(getName() << "Can not unsubscribe a closed subscription, please call "
                           "subscribe again and then call unsubscribe");
    return;
  }
  ...
  if (cnx) {
    ...
    cnx->sendRequestWithId(cmd, requestId)
        .addListener(boost::bind(&ConsumerImpl::handleUnsubscribe, shared_from_this(), _1,
                                 callback));
  } else {
...
void ConsumerImpl::messageReceived(const ClientConnectionPtr& cnx,
...
uint32_t ConsumerImpl::receiveIndividualMessagesFromBatch(const ClientConnectionPtr& cnx,
  ...
  LOG_DEBUG("Received Batch messages of size - "
            << batchSize << " -- msgId: " << batchedMessage.getMessageId());
...
bool ConsumerImpl::uncompressMessageIfNeeded(const ClientConnectionPtr& cnx,
  ...
  CompressionType compressionType =
      CompressionCodecProvider::convertType(metadata.compression());
  ...
  if (payloadSize > Commands::MaxMessageSize) {
    // Uncompressed size is itself corrupted since it cannot be bigger than the
    // MaxMessageSize
  ...
  if (!CompressionCodecProvider::getCodec(compressionType)
           .decode(payload, uncompressedSize, payload)) {
...
void ConsumerImpl::discardCorruptedMessage(
    const ClientConnectionPtr& cnx, const proto::MessageIdData& messageId,
    proto::CommandAck::ValidationError validationError) {
  LOG_ERROR(getName() << "Discarding corrupted message at " << messageId.ledgerid() << ":"
                      << messageId.entryid());

  SharedBuffer cmd = Commands::newAck(consumerId_, messageId,
                                      proto::CommandAck::Individual, validationError);
...
Result ConsumerImpl::fetchSingleMessageFromBroker(Message& msg) {
  if (config_.getReceiverQueueSize() != 0) {
    LOG_ERROR(
        getName() << " Can't use receiveForZeroQueueSize if the queue size is not 0");
  ...
  if (incomingMessages_.size() != 0) {
    LOG_ERROR(getName() << "The incoming message queue should never be greater than 0 "
                           "when Queue size is 0");
    incomingMessages_.clear();
  }
  ...
  while (true) {
    ...
    {
      // Lock needed to prevent race between connectionOpened and the check
      // "msg.impl_->cnx_ == currentCnx.get())"
      Lock localLock(mutex_);
      // if message received due to an old flow - discard it and wait for the message from
      // the
      // latest flow command
...
/**
 * Clear the internal receiver queue and returns the message id of what was the 1st
 * message in the queue that was
...
Optional<BatchMessageId> ConsumerImpl::clearReceiveQueue() {
  ...
  if (incomingMessages_.peekAndClear(nextMessageInQueue)) {
    ...
    // We can safely cast to 'BatchMessageId' since all the messages queued will have that
    // type of message id,
    ...
    if (nextMessageId.batchIndex_ >= 0) {
      ...
      previousMessageId =
          BatchMessageId(nextMessageId.ledgerId_, nextMessageId.entryId_ - 1, -1);
    }
    return Optional<BatchMessageId>::of(previousMessageId);
  } else if (lastDequedMessage_.is_present()) {
    // If the queue was empty we need to restart from the message just after the last one
    // that has been dequeued
    // in the past
    return lastDequedMessage_;
  } else {
    // No message was received or dequeued by this consumer. Next message would still be
    // the startMessageId
...
void ConsumerImpl::acknowledgeAsync(const MessageId& msgId, ResultCallback callback) {
  ResultCallback cb = boost::bind(&ConsumerImpl::statsCallback, this, _1, callback,
...
void ConsumerImpl::doAcknowledge(const BatchMessageId& messageId,
  ...
  if (cnx) {
    ...
    LOG_DEBUG(
        getName() << "Connection is not ready, Acknowledge failed for message - ["  //
...
void ConsumerImpl::getBrokerConsumerStatsAsync(BrokerConsumerStatsCallback callback) {
  ...
  if (brokerConsumerStats_.isValid()) {
    ...
    lock.unlock();
    callback(ResultOk, BrokerConsumerStats(boost::make_shared<BrokerConsumerStatsImpl>(
                           brokerConsumerStats_)));
    return;
  }
  lock.unlock();
  ...
  if (cnx) {
    if (cnx->getServerProtocolVersion() >= proto::v8) {
      ClientImplPtr client = client_.lock();
      uint64_t requestId = client->newRequestId();
      LOG_DEBUG(getName() << " Sending ConsumerStats Command for Consumer - "
                          << getConsumerId() << ", requestId - " << requestId);

      cnx->newConsumerStats(consumerId_, requestId)
          .addListener(boost::bind(&ConsumerImpl::brokerConsumerStatsListener,
                                   shared_from_this(), _1, _2, callback));
      return;
    } else {
      LOG_ERROR(getName() << " Operation not supported since server protobuf version "
                          << cnx->getServerProtocolVersion()
                          << " is older than proto::v7");
...
void ConsumerImpl::brokerConsumerStatsListener(
    Result res, BrokerConsumerStatsImpl brokerConsumerStats,
  ...
  if (!callback.empty()) {
    callback(res, BrokerConsumerStats(
                      boost::make_shared<BrokerConsumerStatsImpl>(brokerConsumerStats)));
  }
}
