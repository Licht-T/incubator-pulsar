...
class BatchAcknowledgementTracker {
 private:
  ...
  // SendList is used to reduce the time required to go over the dynamic_bitset and check
  // if the entire batch is acked.
  // It is useful in cases where the entire batch is acked but cnx is broken. In this case
  // when any of the batch index
  // is acked again, we just check the sendList to verify that the batch is acked w/o
  // iterating over the dynamic_bitset.
 ...
 public:
  ...
  inline friend std::ostream& operator<<(
      std::ostream& os, const BatchAcknowledgementTracker& batchAcknowledgementTracker);
  ...
  struct TrackerMapRemoveCriteria {
   ...
   public:
    ...
    bool operator()(std::pair<const pulsar::BatchMessageId, boost::dynamic_bitset<> >&
                        element) const {
