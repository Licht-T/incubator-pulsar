...
void PartitionedProducerImpl::start() {
  ...
  for (unsigned int i = 0; i < numPartitions_; i++) {
    ...
    producer->getProducerCreatedFuture().addListener(
        boost::bind(&PartitionedProducerImpl::handleSinglePartitionProducerCreated,
                    shared_from_this(), _1, _2, i));
    producers_.push_back(producer);
    LOG_DEBUG("Creating Producer for single Partition - " << topicPartitionName);
  }

  for (ProducerList::const_iterator prod = producers_.begin(); prod != producers_.end();
       prod++) {
    (*prod)->start();
  }
}
...
void PartitionedProducerImpl::handleSinglePartitionProducerCreated(
    Result result, ProducerImplBaseWeakPtr producerWeakPtr, unsigned int partitionIndex) {
  ...
  if (result != ResultOk) {
    ...
    LOG_DEBUG("Unable to create Producer for partition - " << partitionIndex
                                                           << " Error - " << result);
    return;
  }
...
void PartitionedProducerImpl::sendAsync(const Message& msg, SendCallback callback) {
  ...
  if (partition >= numPartitions_ || partition >= producers_.size()) {
    LOG_ERROR("Got Invalid Partition for message from Router Policy, Partition - "
              << partition);
...
/*
 * if createProducerCallback is set, it means the closeAsync is called from CreateProducer
 * API which failed to create
...
void PartitionedProducerImpl::closeAsync(CloseCallback closeCallback) {
  ...
  for (ProducerList::const_iterator i = producers_.begin(); i != producers_.end(); i++) {
    ...
    if (!prod->isClosed()) {
      prod->closeAsync(
          boost::bind(&PartitionedProducerImpl::handleSinglePartitionProducerClose,
   ...
   * a. If closeAsync before creation then state == Closed, since producers_.size() =
   * producerAlreadyClosed = 0
   ...
   * c. If closeAsync called due to failure in creating just one sub producer then state
   * is set by handleSinglePartitionProducerCreated
   */
...
void PartitionedProducerImpl::handleSinglePartitionProducerClose(
    Result result, const unsigned int partitionIndex, CloseCallback callback) {
  Lock lock(mutex_);
  if (state_ == Failed) {
