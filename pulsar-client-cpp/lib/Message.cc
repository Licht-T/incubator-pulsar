...
Message::Message(const proto::CommandMessage& msg, proto::MessageMetadata& metadata,
  ...
  impl_->messageId =
      BatchMessageId(msg.message_id().ledgerid(), msg.message_id().entryid());
  impl_->metadata = metadata;
  impl_->payload = payload;
}

Message::Message(const BatchMessageId& messageID, proto::MessageMetadata& metadata,
...
std::ostream& operator<<(std::ostream& s, const Message& msg) {
  ...
  s << "Message(prod=" << msg.impl_->metadata.producer_name()
    ...
    << ", publish_time=" << msg.impl_->metadata.publish_time()
    << ", payload_size=" << msg.getLength() << ", msg_id=" << msg.getMessageId()
