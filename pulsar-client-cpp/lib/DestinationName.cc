...
void DestinationName::parse(const std::string& destinationName, std::string& domain,
                            std::string& property, std::string& cluster,
                            std::string& namespacePortion, std::string& localName) {
  ...
  slashIndex++;
  localName =
      destinationNameCopy.substr(slashIndex, (destinationNameCopy.size() - slashIndex));
}
std::string DestinationName::getEncodedName(const std::string& nameBeforeEncoding) {
  ...
  if (getCurlHandle()) {
    char* encodedName = curl_easy_escape(getCurlHandle(), nameBeforeEncoding.c_str(),
                                         nameBeforeEncoding.size());
    if (encodedName) {
      nameAfterEncoding.assign(encodedName);
      curl_free(encodedName);
    } else {
      LOG_ERROR("Unable to encode the name using curl_easy_escape, name - "
                << nameBeforeEncoding);
    }
  } else {
...
// TODO - for now return empty string if there's any error in format, later think about
...
const std::string DestinationName::getTopicPartitionName(unsigned int partition) {
  ...
  topicPartitionName << toString() << PartitionedProducerImpl::PARTITION_NAME_SUFFIX
                     << partition;
