...
void BatchAcknowledgementTracker::receivedMessage(const Message& message) {
  ...
  // ignore message if it is less than the last cumulative ack sent or messageID is
  ...
  // Since dynamic_set (this version) doesn't have all() function, initializing all bits
  // with 1 and then reseting them to 0 and using any()
  trackerMap_.insert(
      pos,
      TrackerPair(msgID,
                  boost::dynamic_bitset<>(message.impl_->metadata.num_messages_in_batch())
                      .set()));
}
...
void BatchAcknowledgementTracker::deleteAckedMessage(const BatchMessageId& messageId,
  ...
  if (ackType == proto::CommandAck_AckType_Cumulative) {
    ...
    // equal to - since getGreatestCumulativeAckReady already gives us the exact message
    // id to be acked
    ...
    // std::remove shifts all to be deleted items to the end of the vector and returns an
    // iterator to the first
    // instance of item, then we erase all elements from this iterator to the end of the
    // list
    sendList_.erase(
    ...
    if (trackerMap_.find(messageId) != trackerMap_.end()) {
      LOG_ERROR(*this << " - This should not happened - Message should have been removed "
                         "from trakerMap_ and moved to sendList_ "
                      << messageId);
    }
...
bool BatchAcknowledgementTracker::isBatchReady(const BatchMessageId& msgID,
  ...
  if (pos == trackerMap_.end() ||
      std::find(sendList_.begin(), sendList_.end(), msgID) != sendList_.end()) {
    LOG_DEBUG(
        "Batch is ready since message present in sendList_ or not present in trackerMap_ "
        "[message ID = "
        << msgID << "]");
    return true;
  }
  ...
  LOG_DEBUG(
      "Batch is ready since message all bits are reset in trackerMap_ [message ID = "
      << msgID << "]");
  return true;
}
