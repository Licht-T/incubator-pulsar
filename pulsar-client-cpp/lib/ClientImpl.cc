...
ClientImpl::ClientImpl(const std::string& serviceUrl,
    ...
    : mutex_(),
      ...
      ioExecutorProvider_(boost::make_shared<ExecutorServiceProvider>(
          clientConfiguration.getIOThreads())),
      listenerExecutorProvider_(boost::make_shared<ExecutorServiceProvider>(
          clientConfiguration.getMessageListenerThreads())),
      partitionListenerExecutorProvider_(boost::make_shared<ExecutorServiceProvider>(
          clientConfiguration.getMessageListenerThreads())),
      pool_(clientConfiguration, ioExecutorProvider_, clientConfiguration.getAuthPtr(),
            poolConnections),
  ...
  if (serviceUrl_.compare(0, 4, "http") == 0) {
    ...
    lookupServicePtr_ = boost::make_shared<HTTPLookupService>(
        boost::cref(serviceUrl_), boost::cref(clientConfiguration_),
        boost::cref(clientConfiguration.getAuthPtr()));
  } else {
    LOG_DEBUG("Using Binary Lookup");
    lookupServicePtr_ = boost::make_shared<BinaryProtoLookupService>(
        boost::ref(pool_), boost::ref(serviceUrl));
  }
}
...
void ClientImpl::createProducerAsync(const std::string& topic, ProducerConfiguration conf,
  ...
  lookupServicePtr_->getPartitionMetadataAsync(dn).addListener(boost::bind(
      &ClientImpl::handleCreateProducer, shared_from_this(), _1, _2, dn, conf, callback));
}
...
void ClientImpl::handleCreateProducer(const Result result,
  ...
  if (!result) {
    ...
    if (partitionMetadata->getPartitions() > 1) {
      producer = boost::make_shared<PartitionedProducerImpl>(
          shared_from_this(), dn, partitionMetadata->getPartitions(), conf);
    } else {
      producer =
          boost::make_shared<ProducerImpl>(shared_from_this(), dn->toString(), conf);
    }
    producer->getProducerCreatedFuture().addListener(
        boost::bind(&ClientImpl::handleProducerCreated, shared_from_this(), _1, _2,
                    callback, producer));
    Lock lock(mutex_);
...
void ClientImpl::handleProducerCreated(Result result,
...
void ClientImpl::createReaderAsync(const std::string& topic,
  ...
  lookupServicePtr_->getPartitionMetadataAsync(dn).addListener(
      boost::bind(&ClientImpl::handleReaderMetadataLookup, shared_from_this(), _1, _2, dn,
                  msgId, conf, callback));
}
...
void ClientImpl::handleReaderMetadataLookup(const Result result,
  ...
  if (result != ResultOk) {
    LOG_ERROR(
        "Error Checking/Getting Partition Metadata while creating reader: " << result);
  ...
  if (partitionMetadata->getPartitions() > 1) {
    LOG_ERROR(
        "Topic reader cannot be created on a partitioned topic: " << dn->toString());
    callback(ResultOperationNotSupported, Reader());
    return;
  }

  ReaderImplPtr reader =
      boost::make_shared<ReaderImpl>(shared_from_this(), dn->toString(), conf,
...
void ClientImpl::subscribeAsync(const std::string& topic, const std::string& consumerName,
  ...
  lookupServicePtr_->getPartitionMetadataAsync(dn).addListener(
      boost::bind(&ClientImpl::handleSubscribe, shared_from_this(), _1, _2, dn,
  ...
  if (result == ResultOk) {
    ...
    if (partitionMetadata->getPartitions() > 1) {
      ...
      consumer = boost::make_shared<PartitionedConsumerImpl>(
          shared_from_this(), consumerName, dn, partitionMetadata->getPartitions(), conf);
    } else {
      consumer = boost::make_shared<ConsumerImpl>(shared_from_this(), dn->toString(),
                                                  consumerName, conf);
    }
    consumer->getConsumerCreatedFuture().addListener(
        boost::bind(&ClientImpl::handleConsumerCreated, shared_from_this(), _1, _2,
                    callback, consumer));
    Lock lock(mutex_);
...
Future<Result, ClientConnectionWeakPtr> ClientImpl::getConnection(
...
void ClientImpl::handleLookup(Result result, LookupDataResultPtr data,
  ...
  if (data) {
    ...
    Future<Result, ClientConnectionWeakPtr> future =
        pool_.getConnectionAsync(data->getBrokerUrl());
    future.addListener(
        boost::bind(&ClientImpl::handleNewConnection, this, _1, _2, promise));
  } else {
...
void ClientImpl::closeAsync(CloseCallback callback) {
  ...
  SharedInt numberOfOpenHandlers =
      boost::make_shared<int>(producers.size() + consumers.size());
...
void ClientImpl::handleClose(Result result, SharedInt numberOfOpenHandlers,
  ...
  if (*numberOfOpenHandlers == 0) {
    ...
    if (errorClosing) {
      LOG_DEBUG(
          "Problem in closing client, could not close one or more consumers or "
          "producers");
      if (callback) {
        callback(failResult);
      }
