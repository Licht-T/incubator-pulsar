...
PairSharedBuffer Commands::newSend(SharedBuffer& headers, BaseCommand& cmd,
                                   uint64_t producerId, uint64_t sequenceId,
                                   ChecksumType checksumType, const Message& msg) {
  ...
  // / Wire format
  // [TOTAL_SIZE] [CMD_SIZE][CMD] [MAGIC_NUMBER][CHECKSUM] [METADATA_SIZE][METADATA]
  // [PAYLOAD]
  ...
  int headerContentSize = 4 + cmdSize + magicAndChecksumLength + 4 +
                          msgMetadataSize;  // cmdLength + cmdSize + magicLength +
  ...
  if (includeChecksum) {
    ...
    uint32_t metadataChecksum = computeChecksum(0, headers.data() + metadataStartIndex,
                                                (writeIndex - metadataStartIndex));
    uint32_t computedChecksum =
        computeChecksum(metadataChecksum, payload.data(), payload.writerIndex());
...
SharedBuffer Commands::newSubscribe(const std::string& topic,
                                    const std::string& subscription, uint64_t consumerId,
                                    uint64_t requestId, CommandSubscribe_SubType subType,
...
SharedBuffer Commands::newRedeliverUnacknowledgedMessages(uint64_t consumerId) {
  ...
  CommandRedeliverUnacknowledgedMessages* command =
      cmd.mutable_redeliverunacknowledgedmessages();
...
void Commands::serializeSingleMessageInBatchWithPayload(
    const Message& msg, SharedBuffer& batchPayLoad,
  ...
  if (batchPayLoad.writableBytes() <= sizeof(uint32_t) + msgMetadataSize + payloadSize) {
    LOG_DEBUG("remaining size of batchPayLoad buffer ["
              << batchPayLoad.writableBytes() << "] can't accomodate new payload ["
              << requiredSpace << "] - expanding the batchPayload buffer");
    SharedBuffer buffer = SharedBuffer::allocate(
        batchPayLoad.readableBytes() + std::max(requiredSpace, maxMessageSizeInBytes));
...
Message Commands::deSerializeSingleMessageInBatch(Message& batchedMessage) {
  ...
  Message singleMessage(batchedMessage.impl_->messageId, batchedMessage.impl_->metadata,
                        payload, metadata);
