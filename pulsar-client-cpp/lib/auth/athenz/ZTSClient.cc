...
const static long long FETCH_EPSILON =
    60;  // if cache expires in 60 seconds, get it from ZTS
const static std::string requiredParams[] = {"tenantDomain", "tenantService",
                                             "providerDomain", "privateKey", "ztsUrl"};
...
ZTSClient::ZTSClient(std::map<std::string, std::string> &params) {
  ...
  principalHeader_ = params.find("principalHeader") == params.end()
                         ? DEFAULT_PRINCIPAL_HEADER
                         : params["principalHeader"];
  roleHeader_ = params.find("roleHeader") == params.end() ? DEFAULT_ROLE_HEADER
                                                          : params["roleHeader"];
  ...
  if (params.find("tokenExpirationTime") != params.end()) {
    ...
    if (tokenExpirationTime_ < MIN_TOKEN_EXPIRATION_TIME_SEC) {
      LOG_WARN(tokenExpirationTime_ << " is too small as a token expiration time. "
                                    << MIN_TOKEN_EXPIRATION_TIME_SEC
                                    << " is set instead of it.");
...
const std::string ZTSClient::getPrincipalToken() const {
  ...
  unsignedTokenString +=
      ";e=" + boost::lexical_cast<std::string>(t + tokenExpirationTime_);
  ...
  if (privateKeyUri_.scheme == "data") {
    if (privateKeyUri_.mediaTypeAndEncodingType != "application/x-pem-file;base64") {
      LOG_ERROR("Unsupported mediaType or encodingType: "
                << privateKeyUri_.mediaTypeAndEncodingType);
      return "";
    }
  ...
  std::string principalToken =
      unsignedTokenString + ";s=" + ybase64Encode(signature, siglen);
...
const std::string ZTSClient::getRoleToken() const {
  RoleToken roleToken;
  std::string cacheKey =
      "p=" + tenantDomain_ + "." + tenantService_ + ";d=" + providerDomain_;
  ...
  if (!roleToken.token.empty() &&
      roleToken.expiryTime > (long long)time(NULL) + FETCH_EPSILON) {
    ...
    case CURLE_OK:
      ...
      if (response_code == 200) {
        ...
        if (!reader.parse(responseData, root)) {
          LOG_ERROR("Failed to parse json of ZTS response: "
        ...
        LOG_ERROR("Response failed for url " << completeUrl << ". response Code "
                                             << response_code)
      }
      break;
    default:
