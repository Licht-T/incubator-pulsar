...
OpSendMsg::OpSendMsg()
    : msg_(), sendCallback_(), producerId_(), sequenceId_(), timeout_() {}
...
void ProducerImpl::handleCreateProducer(const ClientConnectionPtr& cnx, Result result,
  ...
  if (result == ResultOk) {
    ...
    if (result == ResultTimeout) {
      // Creating the producer has timed out. We need to ensure the broker closes the
      // producer
      // in case it was indeed created, otherwise it might prevent new create producer
      // operation,
    ...
    if (producerCreatedPromise_.isComplete()) {
      if (result == ResultProducerBlockedQuotaExceededException) {
        LOG_WARN(
            getName() << "Backlog is exceeded on topic. Sending exception to producer");
        failPendingMessages(ResultProducerBlockedQuotaExceededException);
      } else if (result == ResultProducerBlockedQuotaExceededError) {
        LOG_WARN(
            getName()
            << "Producer is blocked on creation because backlog is exceeded on topic");
      }

      // Producer had already been initially created, we need to retry connecting in any
      // case
...
void ProducerImpl::statsCallBackHandler(Result res, const Message& msg,
...
void ProducerImpl::sendAsync(const Message& msg, SendCallback callback) {
  ...
  SendCallback cb =
      boost::bind(&ProducerImpl::statsCallBackHandler, this, _1, _2, callback,
  ...
  if (!batchMessageContainer) {
    // If batching is enabled we compress all the payloads together before sending the
    // batch
    payload =
  ...
  if (compressedSize > Commands::MaxMessageSize) {
    LOG_DEBUG(getName() << " - compressed Message payload size" << compressedSize
                        << "cannot exceed " << Commands::MaxMessageSize << " bytes");
  ...
  if (!conf_.getBlockIfQueueFull() && !pendingMessagesQueue_.tryReserve(1)) {
    ...
    // If queue is full sending the batch immediately, no point waiting till
    // batchMessagetimeout
    if (batchMessageContainer) {
...
void ProducerImpl::handleSendTimeout(const boost::system::error_code& err) {
  ...
  if (!pendingMessagesQueue_.peek(msg)) {
    ...
    // If there is at least one message, calculate the diff between the message timeout
    // and
    // the current time.
    time_duration diff = msg.timeout_ - now();
    if (diff.total_milliseconds() <= 0) {
      // The diff is less than or equal to zero, meaning that the message has been
      // expired.
      LOG_DEBUG(getName() << "Timer expired. Calling timeout callbacks.");
      failPendingMessages(ResultTimeout);
      // Since the pending queue is cleared now, set timer to expire after configured
      // value.
      sendTimer_->expires_from_now(milliseconds(conf_.getSendTimeout()));
    } else {
...
bool ProducerImpl::removeCorruptMessage(uint64_t sequenceId) {
  ...
  if (sequenceId > expectedSequenceId) {
    ...
    LOG_DEBUG(getName() << "Corrupt message is already timed out. Ignoring msg "
                        << sequenceId);
    return true;
  } else {
...
bool ProducerImpl::ackReceived(uint64_t sequenceId) {
  ...
  if (sequenceId > expectedSequenceId) {
    ...
    lastSequenceIdPublished_ =
        sequenceId + op.msg_.impl_->metadata.num_messages_in_batch() - 1;
...
bool ProducerImplCmp::operator()(const ProducerImplPtr& a,
                                 const ProducerImplPtr& b) const {
