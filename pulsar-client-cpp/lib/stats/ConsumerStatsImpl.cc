...
ConsumerStatsImpl::ConsumerStatsImpl(std::string consumerStr, DeadlineTimerPtr timer,
  ...
  timer_->async_wait(boost::bind(&pulsar::ConsumerStatsImpl::flushAndReset, this,
...
void ConsumerStatsImpl::messageAcknowledged(Result res,
...
std::ostream& operator<<(
  ...
  os << "{";
  for (std::map<std::pair<Result, proto::CommandAck_AckType>,
                unsigned long>::const_iterator it = m.begin();
       it != m.end(); it++) {
    os << "[Key: {"
       << "Result: " << strResult((it->first).first)
       << ", ackType: " << (it->first).second << "}, Value: " << it->second << "], ";
  }
  os << "}";
  return os;
}
...
std::ostream& operator<<(std::ostream& os, const ConsumerStatsImpl& obj) {
  os << "Consumer " << obj.consumerStr_ << ", ConsumerStatsImpl ("
     ...
     << ", receivedMsgMap_ = " << obj.receivedMsgMap_
     << ", ackedMsgMap_ = " << obj.ackedMsgMap_
