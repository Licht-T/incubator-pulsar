...
ProducerStatsImpl::ProducerStatsImpl(std::string producerStr, DeadlineTimerPtr timer,
  ...
  timer_->async_wait(boost::bind(&pulsar::ProducerStatsImpl::flushAndReset, this,
...
void ProducerStatsImpl::messageReceived(Result& res,
  ...
  totalSendMap_[res] +=
      1;  // Value will automatically be initialized to 0 in the constructor
}
...
std::ostream& operator<<(std::ostream& os, const ProducerStatsImpl& obj) {
  os << "Producer " << obj.producerStr_ << ", ProducerStatsImpl ("
     ...
     << ", sendMap_ = " << obj.sendMap_ << ", latencyAccumulator_ = "
     << ProducerStatsImpl::latencyToString(obj.latencyAccumulator_)
     << ", totalMsgsSent_ = " << obj.totalMsgsSent_
     << ", totalBytesSent_ = " << obj.totalBytesSent_ << ", totalAcksReceived_ = "
