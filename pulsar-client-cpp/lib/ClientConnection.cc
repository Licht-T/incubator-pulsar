...
void ClientConnection::handlePulsarConnected(const CommandConnected& cmdConnected) {
  ...
  if (serverProtocolVersion_ >= v1) {
    ...
    keepAliveTimer_->expires_from_now(
        boost::posix_time::seconds(KeepAliveIntervalInSeconds));
...
void ClientConnection::startConsumerStatsTimer(
  ...
  for (int i = 0; i < consumerStatsRequests.size(); i++) {
    ...
    if (it != pendingConsumerStatsMap_.end()) {
      ...
      LOG_DEBUG(cnxString_ << "request_id " << it->first
                           << " already fulfilled - not removing it");
    }
  }
...
typedef boost::asio::detail::socket_option::integer<IPPROTO_TCP, TCP_KEEPINTVL>
    tcp_keep_alive_interval;

/// The interval between the last data packet sent (simple ACKs are not considered data)
/// and the first keepalive
/// probe; after the connection is marked to need keepalive, this counter is not used any
/// further
#ifdef __APPLE__
typedef boost::asio::detail::socket_option::integer<IPPROTO_TCP, TCP_KEEPALIVE>
    tcp_keep_alive_idle;
#else
...
void ClientConnection::handleTcpConnected(const boost::system::error_code& err,
  ...
  if (!err) {
    ...
    // Start TCP keep-alive probes after connection has been idle after 1 minute. Ideally
    // this
    // should never happen, given that we're sending our own keep-alive probes (within the
    // TCP
    ...
    socket_->async_connect(
        endpoint, boost::bind(&ClientConnection::handleTcpConnected, shared_from_this(),
...
void ClientConnection::readNextCommand() {
  ...
  asyncReceive(
      ...
      customAllocReadHandler(boost::bind(&ClientConnection::handleRead,
                                         shared_from_this(), _1, _2, minReadSize)));
}

void ClientConnection::handleRead(const boost::system::error_code& err,
                                  size_t bytesTransferred, uint32_t minReadSize) {
  ...
  while (incomingBuffer_.readableBytes() >= sizeof(uint32_t)) {
    ...
    if (frameSize > incomingBuffer_.readableBytes()) {
      ...
      if (bytesToReceive <= incomingBuffer_.writableBytes()) {
        ...
        uint32_t newBufferSize =
            std::max<uint32_t>(DefaultBufferSize, frameSize + sizeof(uint32_t));
    ...
    if (incomingCmd_.type() == BaseCommand::MESSAGE) {
      ...
      if (!msgMetadata.ParseFromArray(incomingBuffer_.data(), metadataSize)) {
        LOG_ERROR(
            cnxString_ << "[consumer id " << incomingCmd_.message().consumer_id()  //
                       << ", message ledger id "
      ...
      handleIncomingMessage(incomingCmd_.message(), isChecksumValid, msgMetadata,
                            payload);
    } else {
...
bool ClientConnection::verifyChecksum(SharedBuffer& incomingBuffer_,
  ...
  if (incomingBuffer_.readUnsignedShort() == Commands::magicCrc32c) {
    ...
    uint32_t computedChecksum =
        computeChecksum(0, incomingBuffer_.data(), metadataPayloadSize);
    // verify checksum
    ...
    if (!isChecksumValid) {
      LOG_ERROR("[consumer id "
                ...
                << incomingCmd_.message().message_id().ledgerid()                  //
                << ", entry id " << incomingCmd_.message().message_id().entryid()  //
                << "stored-checksum" << storedChecksum << "computedChecksum"
                << computedChecksum  //
...
void ClientConnection::handleIncomingMessage(const proto::CommandMessage& msg,
  ...
  if (it != consumers_.end()) {
    ...
    if (consumer) {
      ...
      lock.unlock();
      consumer->messageReceived(shared_from_this(), msg, isChecksumValid, msgMetadata,
                                payload);
    } else {
    ...
    case Ready: {
      ...
      switch (incomingCmd_.type()) {
          ...
          if (it != producers_.end()) {
            ...
            if (producer) {
              if (!producer->ackReceived(sequenceId)) {
                // If the producer fails to process the ack, we need to close the
        ...
        case BaseCommand::PARTITIONED_METADATA_RESPONSE: {
          ...
          LOG_DEBUG(
              cnxString_ << "Received partition-metadata response from server. req_id: "
        ...
        case BaseCommand::CONSUMER_STATS_RESPONSE: {
          ...
          LOG_DEBUG(cnxString_ << "ConsumerStatsResponse command - Received consumer "
                                  "stats response from server. req_id: "
          ...
          if (it != pendingConsumerStatsMap_.end()) {
            ...
            LOG_WARN(
                "ConsumerStatsResponse command - Received unknown request id from "
                "server: "
                << consumerStatsResponse.request_id());
          }
          break;
        ...
        case BaseCommand::PRODUCER_SUCCESS: {
          ...
          LOG_DEBUG(
              cnxString_ << "Received success producer response from server. req_id: "
...
Future<Result, BrokerConsumerStatsImpl> ClientConnection::newConsumerStats(
...
void ClientConnection::newTopicLookup(const std::string& destinationName,
                                      bool authoritative, const uint64_t requestId,
                                      LookupDataResultPromisePtr promise) {
  newLookup(Commands::newLookup(destinationName, authoritative, requestId), requestId,
            promise);
}
...
void ClientConnection::sendCommand(const SharedBuffer& cmd) {
  ...
  if (pendingWriteOperations_++ == 0) {
    ...
    asyncWrite(cmd.const_asio_buffer(),
               customAllocWriteHandler(boost::bind(&ClientConnection::handleSend,
                                                   shared_from_this(), _1, cmd)));
  } else {
    // Queue to send later
...
void ClientConnection::sendMessage(const OpSendMsg& opSend) {
  ...
  if (pendingWriteOperations_++ == 0) {
    PairSharedBuffer buffer =
        Commands::newSend(outgoingBuffer_, outgoingCmd_, opSend.producerId_,
                          opSend.sequenceId_, getChecksumType(), opSend.msg_);

    // Write immediately to socket
    asyncWrite(buffer, customAllocWriteHandler(boost::bind(
                           &ClientConnection::handleSendPair, shared_from_this(), _1)));
  } else {
    // Queue to send later
...
void ClientConnection::handleSend(const boost::system::error_code& err,
                                  const SharedBuffer&) {
  if (err) {
...
void ClientConnection::handleSendPair(const boost::system::error_code& err) {
  if (err) {
    LOG_WARN(cnxString_ << "Could not send pair message on connection: " << err << " "
                        << err.message());
    close();
  } else {
    sendPendingCommands();
...
Future<Result, ResponseData> ClientConnection::sendRequestWithId(SharedBuffer cmd,
                                                                 int requestId) {
  Lock lock(mutex_);
  ...
  requestData.timer->async_wait(boost::bind(&ClientConnection::handleRequestTimeout,
                                            shared_from_this(), _1, requestData));
...
void ClientConnection::handleConsumerStatsTimeout(
    const boost::system::error_code& ec, std::vector<uint64_t> consumerStatsRequests) {
  if (ec) {
...
void ClientConnection::close() {
  ...
  for (PendingRequestsMap::iterator it = pendingRequests_.begin();
