...
TEST(BasicEndToEndTest, testIamSoFancyCharactersInTopicName) {
  ...
  Producer producer;
  Result result =
      client.createProducer("persistent://prop/unit/ns1/topic@%*)(&!%$#@#$><?", producer);
  ASSERT_EQ(ResultOk, result);

  Consumer consumer;
  result = client.subscribe("persistent://prop/unit/ns1/topic@%*)(&!%$#@#$><?",
...
TEST(BasicEndToEndTest, testPartitionedProducerConsumer) {
  ...
  std::string url =
      adminUrl +
      "admin/persistent/prop/unit/ns/testPartitionedProducerConsumer/partitions";
...
TEST(BasicEndToEndTest, testPartitionedProducerConsumerSubscriptionName) {
  ...
  std::string url = adminUrl +
                    "admin/persistent/prop/unit/ns/"
                    "testPartitionedProducerConsumerSubscriptionName/partitions";
  int res = makePutRequest(url, "3");
  ...
  result = client.subscribe(topicName + "-partition-0", "subscription-A",
                            individualPartitionConsumer);
  ASSERT_EQ(ResultConsumerBusy, result);
...
TEST(BasicEndToEndTest, testSinglePartitionRoutingPolicy) {
  Client client(lookupUrl);
  std::string topicName =
      "persistent://prop/unit/ns/partition-testSinglePartitionRoutingPolicy";

  // call admin api to make it partitioned
  std::string url = adminUrl +
                    "admin/persistent/prop/unit/ns/"
                    "partition-testSinglePartitionRoutingPolicy/partitions";
...
TEST(BasicEndToEndTest, testNamespaceName) {
  boost::shared_ptr<NamespaceName> nameSpaceName =
      NamespaceName::get("property", "bf1", "nameSpace");
...
TEST(BasicEndToEndTest, testDuplicateConsumerCreationOnPartitionedTopic) {
  Client client(lookupUrl);
  std::string topicName =
      "persistent://prop/unit/ns/"
      "partition-testDuplicateConsumerCreationOnPartitionedTopic";

  // call admin api to make it partitioned
  std::string url = adminUrl +
                    "admin/persistent/prop/unit/ns/"
                    "testDuplicateConsumerCreationOnPartitionedTopic/partitions";
  int res = makePutRequest(url, "5");
...
TEST(BasicEndToEndTest, testRoundRobinRoutingPolicy) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    subscribeResult = client.subscribe(
        partitionedTopicName.str(), partitionedConsumerId.str(), consConfig, consumer[i]);
...
TEST(BasicEndToEndTest, testMessageListener) {
  ...
  // call admin api to make it partitioned
  std::string url =
      adminUrl + "admin/persistent/prop/unit/ns/partition-testMessageListener/partitions";
  int res = makePutRequest(url, "5");
...
TEST(BasicEndToEndTest, testMessageListenerPause) {
  Client client(lookupUrl);
  std::string topicName =
      "persistent://property/cluster/namespace/partition-testMessageListenerPause";

  // call admin api to make it partitioned
  std::string url = adminUrl +
                    "admin/persistent/property/cluster/namespace/"
                    "partition-testMessageListener-pauses/partitions";
  int res = makePutRequest(url, "5");
...
TEST(BasicEndToEndTest, testResendViaSendCallback) {
  ...
  std::string topicName =
      "persistent://my-property/my-cluster/my-namespace/testResendViaListener";
  ...
  // On receiving timeout send the message using the Pulsar client IO thread via cb
  // function.
  for (int i = 0; i < 10000; i++) {
    producer.sendAsync(MessageBuilder().build(),
                       boost::bind(resendMessage, _1, _2, producer));
...
TEST(BasicEndToEndTest, testStatsLatencies) {
  ...
  boost::accumulators::detail::extractor_result<
      LatencyAccumulator, boost::accumulators::tag::extended_p_square>::type
...
TEST(BasicEndToEndTest, testHandlerReconnectionLogic) {
  ...
  for (int i = 0; i < numOfMessages; i++) {
    ...
    if (i % 3 == 1) {
      ...
      do {
        ClientConnectionWeakPtr clientConnectionWeakPtr =
            PulsarFriend::getClientConnection(pImpl);
  ...
  ClientConnectionWeakPtr clientConnectionWeakPtr =
      PulsarFriend::getClientConnection(cImpl);
