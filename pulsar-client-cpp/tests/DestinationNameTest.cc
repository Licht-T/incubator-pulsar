...
TEST(DestinationNameTest, testDestinationNameWithSlashes) {
  ...
  dn = DestinationName::get(
      "persistent://property/cluster/namespace/destination/ends/with/slash/");
  ASSERT_TRUE(dn != NULL);
  ASSERT_EQ(DestinationName::getEncodedName("destination/ends/with/slash/"),
            dn->getEncodedLocalName());

  dn = DestinationName::get(
      "persistent://property/cluster/namespace/`~!@#$%^&*()-_+=[]{}|\\;:'\"<>,./?");
  ASSERT_TRUE(dn != NULL);
  ...
  dn = DestinationName::get(
      "persistent://property/cluster/namespace/destination//with//double//slash//");
  ASSERT_TRUE(dn != NULL);
  ASSERT_EQ(DestinationName::getEncodedName("destination//with//double//slash//"),
            dn->getEncodedLocalName());

  dn = DestinationName::get(
      "persistent://property/cluster/namespace//destination/starts/with/slash/");
  ASSERT_TRUE(dn != NULL);
