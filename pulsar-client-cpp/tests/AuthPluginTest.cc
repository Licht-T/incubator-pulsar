...
static void sendCallBackTls(Result r, const Message& msg) {
  ...
  std::string messageContent =
      prefix + boost::lexical_cast<std::string>(globalTestTlsMessagesCounter++);
...
TEST(AuthPluginTest, testTls) {
  ...
  std::string params =
      "tlsCertFile:../../pulsar-broker/src/test/resources/authentication/tls/"
      "client-cert.pem,tlsKeyFile:../../pulsar-broker/src/test/resources/authentication/"
      "tls/client-key.pem";
...
void mockZTS() {
  ...
  boost::asio::ip::tcp::acceptor acceptor(
      io, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 9999));
