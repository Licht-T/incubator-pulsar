...
TEST(ClientDeduplicationTest, testProducerSequenceAfterReconnect) {
  ...
  std::string topicName =
      "persistent://sample/standalone/ns-dedup-1/testProducerSequenceAfterReconnect-" +
  ...
  Reader reader;
  ASSERT_EQ(client.createReader(topicName, MessageId::earliest(), readerConf, reader),
            ResultOk);
...
TEST(ClientDeduplicationTest, testProducerDeduplication) {
  ...
  ASSERT_EQ(
      producer.send(MessageBuilder().setContent("my-message-0").setSequenceId(0).build()),
      ResultOk);
  ASSERT_EQ(
      producer.send(MessageBuilder().setContent("my-message-1").setSequenceId(1).build()),
      ResultOk);
  ASSERT_EQ(
      producer.send(MessageBuilder().setContent("my-message-2").setSequenceId(2).build()),
      ResultOk);
