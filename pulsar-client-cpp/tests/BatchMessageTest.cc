...
TEST(BatchMessageTest, testProducerTimeout) {
  std::string testName =
      boost::lexical_cast<std::string>(epochTime) + "testProducerTimeout";
...
TEST(BatchMessageTest, testBatchSizeInBytes) {
  std::string testName =
      boost::lexical_cast<std::string>(epochTime) + "testBatchSizeInBytes";
  ...
  while (consumer.receive(receivedMsg, 5000) == ResultOk) {
    ...
    LOG_DEBUG("Received Message with [ content - "
              ...
              << "] [ messageID = " << receivedMsg.getMessageId() << "]");
    ASSERT_LT(
        pulsar::PulsarFriend::getBatchIndex((BatchMessageId&)receivedMsg.getMessageId()),
        2);
    ASSERT_EQ(receivedMsg.getProperty("msgIndex"), boost::lexical_cast<std::string>(i++));
    ASSERT_EQ(expectedMessageContent, receivedMsg.getDataAsString());
    ASSERT_EQ(ResultOk, consumer.acknowledge(receivedMsg));
...
TEST(BatchMessageTest, testSmallReceiverQueueSize) {
  std::string testName =
      boost::lexical_cast<std::string>(epochTime) + "testSmallReceiverQueueSize";
  globalTestBatchMessagesCounter = 0;
  ...
  unsigned long t = consumerStatsImplPtr->getAckedMsgMap().at(
      std::make_pair<Result, proto::CommandAck_AckType>(
          ResultOk, proto::CommandAck_AckType_Individual));
  ASSERT_EQ(t, numOfMessages);
  ASSERT_EQ(PulsarFriend::sum(consumerStatsImplPtr->getAckedMsgMap()), numOfMessages);
  ASSERT_EQ(PulsarFriend::sum(consumerStatsImplPtr->getTotalAckedMsgMap()),
            numOfMessages);
  ASSERT_EQ(PulsarFriend::sum(consumerStatsImplPtr->getReceivedMsgMap()), numOfMessages);
  ASSERT_EQ(PulsarFriend::sum(consumerStatsImplPtr->getTotalReceivedMsgMap()),
            numOfMessages);
  ASSERT_EQ(consumerStatsImplPtr->getTotalNumBytesRecieved(),
            consumerStatsImplPtr->getNumBytesRecieved());
  usleep(20 * 1e6);
  ASSERT_NE(consumerStatsImplPtr->getTotalNumBytesRecieved(),
            consumerStatsImplPtr->getNumBytesRecieved());
  ASSERT_EQ(PulsarFriend::sum(consumerStatsImplPtr->getTotalAckedMsgMap()),
            numOfMessages);
  ASSERT_EQ(PulsarFriend::sum(consumerStatsImplPtr->getTotalReceivedMsgMap()),
...
TEST(BatchMessageTest, testIndividualAck) {
  std::string testName =
      boost::lexical_cast<std::string>(epochTime) + "testIndividualAck";
  ...
  i = 0;
  while (consumer.receive(receivedMsg, 5000) == ResultOk) {
    std::string expectedMessageContent =
        prefix + boost::lexical_cast<std::string>(i + numOfMessages / 2);
    LOG_DEBUG("Received Message with [ content - "
              << receivedMsg.getDataAsString()
              << "] [ messageID = " << receivedMsg.getMessageId() << "]");
...
TEST(BatchMessageTest, testCumulativeAck) {
  std::string testName =
      boost::lexical_cast<std::string>(epochTime) + "testCumulativeAck";
  ...
  ASSERT_EQ(producerStatsImplPtr->getNumBytesSent(),
            consumerStatsImplPtr->getNumBytesRecieved());
  unsigned long t = consumerStatsImplPtr->getAckedMsgMap().at(
      std::make_pair<Result, proto::CommandAck_AckType>(
          ResultOk, proto::CommandAck_AckType_Cumulative));
  ASSERT_EQ(t, 1);
...
TEST(BatchMessageTest, testMixedAck) {
  ...
  while (consumer.receive(receivedMsg, 5000) == ResultOk) {
    std::string expectedMessageContent =
        prefix + boost::lexical_cast<std::string>(i + 10);
    LOG_DEBUG("Received Message with [ content - "
              << receivedMsg.getDataAsString()
              << "] [ messageID = " << receivedMsg.getMessageId() << "]");
    ASSERT_EQ(receivedMsg.getProperty("msgIndex"),
              boost::lexical_cast<std::string>(i++ + 10));
    ASSERT_EQ(expectedMessageContent, receivedMsg.getDataAsString());
...
TEST(BatchMessageTest, testPermits) {
  ...
  ASSERT_LE(ConsumerTest::getNumOfMessagesInQueue(consumer),
            consumerConfig.getReceiverQueueSize());
  ASSERT_GE(ConsumerTest::getNumOfMessagesInQueue(consumer),
            consumerConfig.getReceiverQueueSize() / 2);
