...
TEST(BackoffTest, firstBackoffTimerTest) {
  ...
  boost::posix_time::ptime firstBackOffTime = PulsarFriend::getFirstBackoffTime(backoff);
  usleep(300 * 1000);
  TimeDuration diffBackOffTime =
      PulsarFriend::getFirstBackoffTime(backoff) - firstBackOffTime;
  ...
  ASSERT_TRUE(diffBackOffTime >= milliseconds(300) &&
              diffBackOffTime < milliseconds(310));
}
