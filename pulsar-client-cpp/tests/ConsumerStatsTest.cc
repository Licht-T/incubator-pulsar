...
void partitionedCallbackFunction(Result result, BrokerConsumerStats brokerConsumerStats,
                                 long expectedBacklog, Latch& latch, int index) {
...
void simpleCallbackFunction(Result result, BrokerConsumerStats brokerConsumerStats,
...
TEST(ConsumerStatsTest, testBacklogInfo) {
  ...
  client.subscribeAsync(topicName, subName, conf,
  ...
  consumer.getBrokerConsumerStatsAsync(boost::bind(
      simpleCallbackFunction, _1, _2, ResultOk, numOfMessages, ConsumerExclusive));
...
TEST(ConsumerStatsTest, testCachingMechanism) {
  long epochTime = time(NULL);
  std::string testName =
      "testCachingMechanism-" + boost::lexical_cast<std::string>(epochTime);
  Client client(lookupUrl);
  std::string topicName = "persistent://property/cluster/namespace/" + testName;
  std::string subName = "subscription-name";
  ConsumerConfiguration conf;
...
TEST(ConsumerStatsTest, testAsyncCallOnPartitionedTopic) {
  long epochTime = time(NULL);
  std::string testName =
      "testAsyncCallOnPartitionedTopic-" + boost::lexical_cast<std::string>(epochTime);
  Client client(lookupUrl);
  std::string topicName = "persistent://property/cluster/namespace/" + testName;
  std::string subName = "subscription-name";

  // call admin api to create partitioned topics
  std::string url = adminUrl + "admin/persistent/property/cluster/namespace/" + testName +
                    "/partitions";
  ...
  consumer.getBrokerConsumerStatsAsync(
      boost::bind(partitionedCallbackFunction, _1, _2, 5, latch, 0));
  ...
  consumer.getBrokerConsumerStatsAsync(
      boost::bind(partitionedCallbackFunction, _1, _2, 10, latch, 2));
  ...
  consumer.getBrokerConsumerStatsAsync(
      boost::bind(partitionedCallbackFunction, _1, _2, 10, latch, 3));
