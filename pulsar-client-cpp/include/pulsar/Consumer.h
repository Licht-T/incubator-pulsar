...
class Consumer {
 public:
   ...
   * ConsumerShared.
   *
   * It's equivalent to calling asyncAcknowledgeCumulative(const Message&, ResultCallback)
   * and
   * waiting for the callback to be triggered.
   *
   * @param message the last message in the stream to acknowledge
   * @return ResultOk if the message was successfully acknowledged. All previously
   * delivered messages for this topic are also acknowledged.
   * @return ResultError if there was a failure
   */
  ...
  /*
   * Pause receiving messages via the messageListener, till resumeMessageListener() is
   * called.
   */
   ...
   * Asynchronously receive all the messages enqueued from time pauseMessageListener() was
   * called.
   */
  Result resumeMessageListener();

  /**
   * Redelivers all the unacknowledged messages. In Failover mode, the request is ignored
   * if the consumer is not
   * active for the given topic. In Shared mode, the consumers messages to be redelivered
   * are distributed across all
   * the connected consumers. This is a non blocking call and doesn't throw an exception.
   ...
   * The stats are cached for 30 seconds, if a call is made before the stats returned by
   * the previous call expires
   * then cached data will be returned. BrokerConsumerStats::isValid() function can be
   * used to check if the stats are
   * still valid.
   *
   * @param brokerConsumerStats - if the function returns ResultOk, this object will
   * contain consumer stats
   *
  ...
  * The stats are cached for 30 seconds, if a call is made before the stats returned by
  * the previous call expires
  * then cached data will be returned. BrokerConsumerStats::isValid() function can be used
  * to check if the stats are
  * still valid.
  *
