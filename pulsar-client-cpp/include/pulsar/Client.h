...
class Client {
 public:
  /**
   * Create a Pulsar client object connecting to the specified cluster address and using
  ...
  /**
   * Create a topic reader with given {@code ReaderConfiguration} for reading messages
   * from the specified topic.
   * <p>
   * The Reader provides a low-level abstraction that allows for manual positioning in the
   * topic, without using a
   * subscription. Reader can only work on non-partitioned topics.
   * <p>
   * The initial reader positioning is done by specifying a message id. The options are:
   * <ul>
   * <li><code>MessageId.earliest</code> : Start reading from the earliest message
   * available in the topic
   * <li><code>MessageId.latest</code> : Start reading from the end topic, only getting
   * messages published after the
   * reader was created
   * <li><code>MessageId</code> : When passing a particular message id, the reader will
   * position itself on that
   * specific position. The first message to be read will be the message next to the
   * specified messageId.
   * </ul>
   *
   * @param topic
   ...
   *            The message id where the reader will position itself. The first message
   * returned will be the one after
