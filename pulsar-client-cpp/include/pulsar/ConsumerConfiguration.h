...
class ConsumerConfiguration {
 public:
   ...
   *
   * The consumer receive queue controls how many messages can be accumulated by the
   * Consumer before the
   * application calls receive(). Using a higher value could potentially increase the
   * consumer throughput
   * at the expense of bigger memory utilization.
   *
   * Setting the consumer queue size as zero decreases the throughput of the consumer, by
   * disabling pre-fetching of
   * messages. This approach improves the message distribution on shared subscription, by
   * pushing messages only to
   * the consumers that are ready to process them. Neither receive with timeout nor
   * Partitioned Topics can be
   * used if the consumer queue size is zero. The receive() function call should not be
   * interrupted when
  ...
  /**
   * Set the timeout in milliseconds for unacknowledged messages, the timeout needs to be
   * greater than
   * 10 seconds. An Exception is thrown if the given value is less than 10000 (10
   * seconds).
   * If a successful acknowledgement is not sent within the timeout all the unacknowledged
   * messages are
   * redelivered.
