...
enum ConsumerType {
   ...
   *  will be dispatched according to a round-robin rotation between the connected
   * consumers
   */
  ConsumerShared,

  /** Only one consumer is active on the subscription; Subscription can have N consumers
   *  connected one of which will get promoted to master if the current master becomes
   * inactive
   */
  ConsumerFailover
};
