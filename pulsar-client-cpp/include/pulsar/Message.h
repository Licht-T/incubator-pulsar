...
class Message {
 public:
   ...
   * Properties are application defined key/value pairs that will be attached to the
   * message
   *
   ...
   *
   * The message id can be used to univocally refer to a message without having to keep
  ...
  /**
   * Get the UTC based timestamp in milliseconds referring to when the message was
   * published by the client producer
   */
  uint64_t getPublishTimestamp() const;

  /**
   * Get the event timestamp associated with this message. It is set by the client
   * producer.
   */
